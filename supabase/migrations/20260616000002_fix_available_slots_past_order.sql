-- get_available_slots v4: slot_status CASE sırası düzeltildi.
-- Eski sıra: past → own_reservation → closed → reserved → available
-- Yeni sıra: own_reservation → closed → reserved → past → available
-- Böylece geçmiş tarihli dolu slotlar 'past' yerine 'own_reservation'/'reserved'
-- döner; admin hangi slotun gerçekten boş olduğunu görebilir.
...
BEGIN
  ...
  SELECT
    ...
    s.st,
    -- is_available: sadece gelecek + boş + açık slotlarda true
    (
    ...
    -- slot_status: rezervasyon/kapalı kontrolü past'tan ÖNCE gelir
    CASE
      WHEN EXISTS (
